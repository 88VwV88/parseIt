...
from .make_html_skeleton import make_html_skeleton, SkeletonInformation, RequiredError
from .parse_html import read_html, parse_html, Parsed
...
class App(QWidget):
    def __init__(self, app: QApplication) -> None:
        ...
        self.resBox: QGroupBox | None = None
        self.formBox: QGroupBox | None = None
        self.psrBox: QGroupBox | None = None
    ...
    def add_skeleton(self) -> None:
        if self.psrBox:
            self.psrBox.hide()
        self.information: SkeletonInformation | None = None
        self.accepted = False
    ...
    def populate_information(self) -> None:
        try:
            ...
            if not year[2].text():
                ...
                if not (2000 <= _year <= date.today().year + 1):
                    raise AttributeError(
                        f"year must be in [2000, {date.today().year + 1}]"
                    )
            year[2].setText("")
            ...
            if _stylesheet and not _stylesheet.endswith(
                (".tss", "css", ".scss", ".sass")
            ):
                _stylesheet += ".css"

            _filename: str = filename[2].text()
            filename[2].setText("")
            if not _filename:
            ...
            self.information = SkeletonInformation(
                name=_name,
                year=_year,
                title=_title,
        ...
        make_html_skeleton(self.information)

    def populate_tree(self, data: Parsed, parent: QTreeWidget | QTreeWidgetItem):
